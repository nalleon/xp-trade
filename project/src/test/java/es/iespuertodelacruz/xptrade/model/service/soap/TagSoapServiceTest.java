...
public class TagSoapServiceTest extends TestUtilities {
    @Mock
    ITagEntityRepository repositoryMock;

    @InjectMocks
    TagEntityService serviceMock;


    @Mock
    TagSoapService service;

    @BeforeEach
    public void beforeEach (){
        MockitoAnnotations.openMocks(this);
        serviceMock = new TagEntityService();
        serviceMock.setRepository(repositoryMock);
        service = new TagSoapService();
    ...
    @Test
    void getOneTest() {
        when(repositoryMock.findById(1)).thenReturn(Optional.of(new TagEntity()));
    ...
    @Test
    void getByNameTest() {
        when(repositoryMock.findByName(NAME)).thenReturn(Optional.of(new TagEntity()));
    ...
    void addTest() {
        ...
        when(repositoryMock.save(any(TagEntity.class))).thenReturn(new TagEntity());
...
//    }
    ...
    void addExceptionTest() throws Exception {
        when(repositoryMock.save(any(TagEntity.class))).thenThrow(new RuntimeException());
        Assertions.assertThrows(RuntimeException.class, () -> service.save(new Tag(1)), MESSAGE_ERROR);
    }
    ...
    void updateTest() throws Exception {
        ...
        role.setId(1);
        role.setName(NAME);
        when(repositoryMock.findByName(role.getName())).thenReturn(Optional.of(new TagEntity()));
...
//    }
    ...
    void deleteAdminTest() {
        when(repositoryMock.findById(1)).thenReturn(Optional.of(new TagEntity("ROLE_ADMIN")));
